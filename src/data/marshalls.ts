...
/**
 * Wallpaper previews sourced from the public Marshalls India Shopify product
 * feed (`/products.json`). Images are served from Shopify's CDN, which sends
 * `Access-Control-Allow-Origin: *`, so they load as cross-origin WebGL textures
 * without tainting the canvas (snapshots keep working).
 *
 * IMPORTANT — licensing: these are Marshalls India's copyrighted product
 * photos, included here only as demo/preview content so the catalog isn't
 * empty. They are NOT seamless tiles (they're catalogue shots), so they're
 * flagged `tileable: false` and rendered as a single image stretched across the
 * wall. For any production/commercial use you must obtain rights from the
 * owner or swap in your own licensed, tileable textures.
 */
...
function marshalls(sku: string, file: string, version: string): Wallpaper {
  return {
    ...
    physicalRepeatCm: [104, 104], // Marshalls rolls are ~1.04 m wide
    accent: "#6ea8fe",
    tileable: false,
